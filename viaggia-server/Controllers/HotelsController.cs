        ...
        public async Task<IActionResult> CreateHotel([FromForm] CreateHotelDTO createHotelDto)
        {
            ...
            try
            {
                ...
                var hotel = new Hotel
                {
                    ...
                    Cnpj = createHotelDto.Cnpj // Added based on DB schema in logs
                };
                ...
                return CreatedAtAction(
                    nameof(GetHotelById),
                    new { id = createdHotel.HotelId },
                    new ApiResponse<Hotel>(true, "Hotel created successfully.", createdHotel)
                );
            }
            catch (Exception ex)
            {
                return StatusCode(StatusCodes.Status500InternalServerError,
                    new ApiResponse<Hotel>(false, $"Error creating hotel: {ex.Message}"));
            }
        }
