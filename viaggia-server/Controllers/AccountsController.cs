...
namespace viaggia_server.Controllers
{
    ...
    public class AccountsController : ControllerBase
    {
        ...
        public AccountsController(IAuthRepository authRepository, IGoogleAccountRepository repository, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> GoogleCallback()
        {
            ...
            var frontendBaseUrl = _configuration["FRONTEND__BaseUrl"] ?? "http://localhost:5173";
            var redirectUrl = $"{frontendBaseUrl}/auth-success?token={Uri.EscapeDataString(token)}&name={Uri.EscapeDataString(user.Name)}&email={Uri.EscapeDataString(user.Email)}&needsProfileCompletion={string.IsNullOrEmpty(user.PhoneNumber)}";
